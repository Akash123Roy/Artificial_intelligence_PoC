...
if option == "Excel":
    ...
    if file:
        if st.button("Upload and Generate Summary"):
            ...
            # Create a DataFrame to store results with prompts and summaries
            results_df = pd.DataFrame(columns=['Prompt', 'Summary Heading', 'Summary'])
            ...
            for index, row in df.iterrows():
                for i, prompt in enumerate(prompts):
                    ...
                    summary_heading = f'Summary {i+1}'
                    
                    # Append to results DataFrame
                    results_df.loc[len(results_df)] = {
                        'Prompt': prompt_text,
                        'Summary Heading': summary_heading,
                        'Summary': summary
                    }
